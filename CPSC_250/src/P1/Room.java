...
public class Room {
	...
	// private static boolean fileIsWritten = false;
	...
	public static boolean equals(char[][] room1, char[][] room2) {
		if (room1 == null || room2 == null)
			return false; // always assume null arrays are false

		if (room1.length != room2.length)
			return false; // different sizes

		if (room1[0].length != room2[0].length)
			return false; // different sizes
	...
	public static int countGhosts(char[][] room) {
		...
		for (int i = 0; i < room.length; i++)
			for (int j = 0; j < room[0].length; j++)
				if (room[i][j] == GHOST)
					++count; // increment the ghost counter
	 ...
	 * 
	 * @param room
	 *            - room defined as 2D array of characters
	 */
	 ...
	 * @param numGhosts
	 *            - number of randomly placed ghosts to create
	 * @param numWalls
	 *            - number of randomly placed walls to create
	...
	/**
	 * Create a room of predetermined size and fixed positions of ghosts and
	 * walls
	 * 
	 * @param room
	 *            - selection number of the room (1, 2, 3 rooms defined)
	 * @return a 2D character array representing the defined room
	 */
	...
	public static char[][] create5x5Room() {
		...
		/*
		 * originally defined (0,0)+_____+ |·····| |·WW··| |·G··G| |··WW·|
		 * |G·G·W| +-----+
		 */
		room[2][1] = GHOST;
		room[2][4] = GHOST;
	...
	public static boolean writeRoom(String filename, char[][] room) {
		try {
			if (room == null) {
				return false;
			}
	...
	public static char[][] readRoom(String filename) {
		try {
			...
			String sizeLine = scnr.nextLine();
			Scanner sizeOfLine = new Scanner(sizeLine);
			char[][] myArray = null;
			int lineLength = 0;
			while (sizeOfLine.hasNext()) {
				lineLength++;
			}
			...
			while (scnr.hasNextLine()) {
				String ghostLine = scnr.nextLine();

				Scanner ghostRow = new Scanner(ghostLine);
				int heightCount = 0;
				while (ghostRow.hasNext()) {
					char nextIndex = ghostRow.next().charAt(0);
					myArray[width][heightCount++] = nextIndex;
					// heightCount++;
				}
				ghostRow.close();
				myArray = new char[width][heightCount];
				if (heightCount != myArray[width].length) {
					scnr.close();
					return myArray;
				}
				width++;
			}
			if (width != myArray.length) {
				scnr.close();
				return myArray;
			}
			scnr.close();
			//setFileIsWritten(true);
			writeRoom(filename, myArray);
			return myArray;
		} catch (Exception e) {
