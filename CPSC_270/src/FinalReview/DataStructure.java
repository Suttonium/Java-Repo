...
public class DataStructure implements DataStructureADT {
    ...
    /**
     * removes a node if and only if this node is indeed a leaf node
     */
    public void removeLeaf(Character c) {
        removeLeafHelper(root, c);
    }

    private void removeLeafHelper(Node n, Character c) {
        if (isLeafANode(n, c)) {
            remove(c);
        }
    }
    ...
    /**
     * locates a node and determines if it is a leaf node
     * 
     * @param n
     ...
     */
    private Boolean isLeafANode(Node n, Character c) {
        if (n != null) {
            if (n.getValue().compareTo(c) < 0) {
                return isLeafANode(n.getRight(), c);
            }
            else if (n.getValue().compareTo(c) > 0) {
                return isLeafANode(n.getLeft(), c);
            }
            else {
